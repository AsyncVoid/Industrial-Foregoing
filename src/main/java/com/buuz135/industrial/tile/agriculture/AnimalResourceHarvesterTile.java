...
public class AnimalResourceHarvesterTile extends WorkingAreaElectricMachine {
    ...
    public float work() {
        ...
        List<EntitySheep> animals = this.worldObj.getEntitiesWithinAABB(EntitySheep.class, getWorkingArea());
        for (EntitySheep sheep : animals) {
            if (!sheep.getSheared()) {
                List<ItemStack> stacks = sheep.onSheared(new ItemStack(Items.SHEARS), this.worldObj, null, 0);
        ...
        for (EntityCow cow : this.worldObj.getEntitiesWithinAABB(EntityCow.class, getWorkingArea())) {
            FakePlayer player = IndustrialForegoing.getFakePlayer(this.worldObj);
            ItemStack stack = new ItemStack(Items.BUCKET);
            player.setHeldItem(EnumHand.MAIN_HAND, stack);
            if (cow.processInteract(player, EnumHand.MAIN_HAND, stack)) { //TODO processInteract(player, EnumHand.MAIN_HAND)
                //ItemStack stack = player.getHeldItem(EnumHand.MAIN_HAND);
                if (stack.hasCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, null)) { //TODO FLUID_HANDLER_ITEM_CAPABILITY
                    IFluidHandler fluidHandlerItem = stack.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, null);
